...
class Editor {
    ...
    private _selectedElement: number = 0;
    ...
    constructor(public $screen: any, public app: App) {
        this.song = new Song();
    ...
    initIntControl(selector: string, fieldName: string, callback?: any) {
        var self = this;
        this.$screen.on("click", selector + ' .increment, ' + selector + ' .decrement', function (e) {
            e.preventDefault();
            let fieldSelector : string = fieldName;
            let item = self;
    ...
    scrollSequenceEditor($sequenceRow?: any, $sequenceContainer?: any) {
    ...
    scrollTrackEditor($patternCell?: any, $trackEditor?: any) {
    ...
    stepForward(increment: number = 1) {
    ...
    beepNote(note: Note, duration?: number) {
